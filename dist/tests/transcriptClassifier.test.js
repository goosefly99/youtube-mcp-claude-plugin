...
describe("classifyTranscriptError", () => {
    it("classifies 'No captions available' as missing / no-captions", () => {
        const result = classifyTranscriptError(new Error("No captions available for video"));
        assert.strictEqual(result.status, "missing");
        assert.strictEqual(result.reason, "no-captions");
    });
    it("classifies 'Captions disabled' as missing / no-captions", () => {
        const result = classifyTranscriptError(new Error("Captions disabled on this video"));
        assert.strictEqual(result.status, "missing");
        assert.strictEqual(result.reason, "no-captions");
    });
    it("classifies 'HTTP 404' as missing / http-404", () => {
        const result = classifyTranscriptError(new Error("HTTP 404 transcript not found"));
        assert.strictEqual(result.status, "missing");
    ...
    it("classifies unknown errors as failed with truncated reason (≤200 chars)", () => {
        ...
        assert.ok(result.reason.length <= 200, `reason length must be ≤200, got ${result.reason.length}`);
    });
    ...
    it("drift guard: running twice with the same fixture returns identical output", () => {
        ...
        assert.deepStrictEqual(first, second, "classifier must be deterministic — single source of truth");
    });
...
//# sourceMappingURL=transcriptClassifier.test.js.map