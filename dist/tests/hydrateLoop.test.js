...
const tmpDbPath = path.join(os.tmpdir(), `yt-hydrateLoop-${process.pid}-${Date.now()}.db`);
...
describe("hydrate loop reunification (delegates to fetchAndStoreVideo)", () => {
    ...
    it("delegates per-video work to fetchAndStoreVideo using preFetchedDetails (no extra videos.list calls)", async () => {
        ...
        const fetchSpy = mock.method(globalThis, "fetch", async (_input, _init) => {
            throw new Error("globalThis.fetch must NOT be called during the Step 2 transcript pass " +
        ...
        const mockTranscriptFetcher = async (id) => {
            transcriptCalls.push(id);
            if (id === "hlv1" || id === "hlv2")
                return makeTranscript(id);
            if (id === "hlv3")
        ...
        // guard.
        assert.strictEqual(fetchSpy.mock.callCount(), 0, "globalThis.fetch must not be invoked when preFetchedDetails is supplied " +
            "(would indicate a per-video videos.list regression)");
        // Assertion 5: DB rows carry channel_id populated from preFetchedDetails.
        const db = getDb();
        for (const id of videoIds) {
            const row = db
                .prepare("SELECT channel_id, transcript_status FROM videos WHERE video_id = ?")
                .get(id);
            assert.ok(row, `row for ${id} must exist`);
            assert.strictEqual(row.channel_id, `UC_${id}`, `channel_id must be populated for ${id}`);
        }
    });
    it("preserves HYDRATE_TRANSCRIPT_CONCURRENCY=1 (serial ordering)", async () => {
        const { HYDRATE_TRANSCRIPT_CONCURRENCY } = await import("../tools/get-playlist-items.js");
        assert.strictEqual(HYDRATE_TRANSCRIPT_CONCURRENCY, 1);
    });
});
//# sourceMappingURL=hydrateLoop.test.js.map