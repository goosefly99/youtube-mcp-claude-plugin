...
describe("schema defaults (Y4)", () => {
    describe("get_video_details", () => {
        it("includeTranscript defaults to true when omitted", () => {
            ...
            assert.strictEqual(parsed.includeTranscript, true, "includeTranscript must default to true");
        });
    ...
    describe("get_playlist_items", () => {
        it("hydrate defaults to true when omitted", () => {
            ...
            assert.strictEqual(parsed.hydrate, true, "hydrate must default to true");
        });
    ...
    describe("HYDRATE_TRANSCRIPT_CONCURRENCY", () => {
        it("is exported as 1 (serial hydration policy)", () => {
            assert.strictEqual(HYDRATE_TRANSCRIPT_CONCURRENCY, 1, "HYDRATE_TRANSCRIPT_CONCURRENCY must equal 1");
        });
    });
});
//# sourceMappingURL=schemaDefaults.test.js.map