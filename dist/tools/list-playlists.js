...
export function registerListPlaylistsTool(server) {
    server.tool("list_user_playlists", "List all playlists owned by the authenticated YouTube account. Requires OAuth credentials (YOUTUBE_OAUTH_TOKEN_PATH).", {
        maxResults: z
            .number()
            .min(1)
            .max(50)
        ...
        try {
            upsertPlaylists(getDb(), playlists.map((pl) => ({
                playlistId: pl.id,
            ...
            process.stderr.write(`youtube-mcp: DB upsert failed (list_user_playlists): ${err}\n`);
        }
...
//# sourceMappingURL=list-playlists.js.map