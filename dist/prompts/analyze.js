import { z } from "zod";
export function registerAnalyzePrompt(server) {
    server.prompt("analyze", "Perform a deep content analysis of a YouTube video", { url: z.string().describe("YouTube video URL or ID") }, async ({ url }) => ({
        messages: [
            {
                role: "user",
                content: {
