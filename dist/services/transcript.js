...
async function fetchCaptionTracks(videoId) {
    ...
    if (!response.ok) {
        if (response.status === 429) {
            throw new Error("Rate limited by YouTube. Please wait a moment and try again.");
        }
        throw new Error(`Failed to fetch video data: HTTP ${response.status}`);
    }
    const data = (await response.json());
    if (data.playabilityStatus?.status !== "OK") {
        throw new Error(`Video unavailable: ${data.playabilityStatus?.reason ?? data.playabilityStatus?.status ?? "unknown error"}`);
    }
    const tracks = data.captions?.playerCaptionsTracklistRenderer?.captionTracks;
    if (!tracks || tracks.length === 0) {
        throw new Error("No captions available for this video. The creator may not have enabled captions.");
    }
...
function selectTrack(tracks, language) {
    if (language) {
        ...
        if (exact)
            return exact;
        const prefix = tracks.find((t) => t.languageCode.startsWith(language.split("-")[0]));
        if (prefix)
            return prefix;
        throw new Error(`No captions found for language "${language}". Available: ${tracks.map((t) => t.languageCode).join(", ")}`);
    }
...
export async function fetchTranscript(videoIdOrUrl, language) {
...
export async function listAvailableLanguages(videoIdOrUrl) {
    const videoId = parseVideoId(videoIdOrUrl);
    const tracks = await fetchCaptionTracks(videoId);
    return tracks.map((track) => ({
