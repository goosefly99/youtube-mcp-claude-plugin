...
function extractThemes(text: string, topN: number = 10): string[] {
  const STOP = new Set([
...
export function registerAnalyzeVideoTool(server: McpServer): void {
  server.tool(
    "analyze_video",
    "Perform a deep analysis of a YouTube video: key themes from the transcript, comment sentiment analysis, content categorization, and key moments.",
    {
    ...
    async ({ videoId, includeComments }) => {
      ...
      const category =
        CATEGORY_MAP[details.categoryId] ?? `Unknown (${details.categoryId})`;
      ...
      if (comments.length > 0) {
        ...
        sections.push(
          ``,
          `## Comment Sentiment (${sentiment.total} comments analyzed)`,
          `Overall: ${sentiment.overall}`,
          `- Positive: ${sentiment.positive} (${pPct}%)`,
          `- Neutral: ${sentiment.neutral} (${neuPct}%)`,
          `- Negative: ${sentiment.negative} (${nPct}%)`,
        );
      ...
      if (transcript) {
        ...
        const chunkSize = 120;
        const moments: { timestamp: string; description: string }[] = [];
        ...
        for (const seg of transcript.segments) {
          if (seg.start >= chunkStart + chunkSize && chunkTexts.length > 0) {
            ...
            if (prevWords.size > 0) {
              ...
              const similarity =
                overlap / Math.max(prevWords.size, currentWords.size);
              if (similarity < 0.3) {
        ...
        if (moments.length > 0) {
          sections.push(
            ``,
            `## Key Moments (topic shifts)`,
            ...moments.map((m) => `- [${m.timestamp}] ${m.description}`)
          );
        }
        ...
        sections.push(
          ...
          `Length: ${transcript.segments.length} segments, ${transcript.fullText.length} characters`
        );
      } else {
