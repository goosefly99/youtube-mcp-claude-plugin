...
export function registerManagePlaylistTools(server: McpServer): void {
  // ── add_to_playlist ────────────────────────────────────────────────────────

  server.tool(
    "add_to_playlist",
    "Add a video to a YouTube playlist. Accepts a video ID or full YouTube URL. Optionally specify a 0-based position. Requires OAuth credentials (YOUTUBE_OAUTH_TOKEN_PATH).",
    {
      playlistId: z
        .string()
    ...
    async ({ playlistId, videoId, position }) => {
      ...
      const result = await mutateOAuthApi(
        "POST",
        "playlistItems",
        { part: "snippet" },
        { snippet }
      ...
      const itemId = (result as Record<string, string> | null)?.id ?? "unknown";
  ...
  server.tool(
    "remove_from_playlist",
    "Remove a video from a YouTube playlist by its playlist item ID (not the video ID). Use get_playlist_items to obtain item IDs. Requires OAuth credentials (YOUTUBE_OAUTH_TOKEN_PATH).",
    {
      playlistItemId: z
        .string()
        .describe(
          "Playlist item ID to remove (the 'Item ID' field from get_playlist_items)"
        ),
    },
  ...
  server.tool(
    "move_playlist_item",
    "Move a video to a new position within a YouTube playlist. Use get_playlist_items to obtain the playlist item ID. Position is 0-based. Requires OAuth credentials (YOUTUBE_OAUTH_TOKEN_PATH).",
    {
      playlistId: z
        .string()
    ...
    async ({ playlistId, playlistItemId, newPosition }) => {
      ...
      const items = (current.items as Array<{ snippet: Record<string, unknown> }>) ?? [];
      if (items.length === 0) {
      ...
      await mutateOAuthApi(
        "PUT",
        "playlistItems",
        { part: "snippet" },
        { id: playlistItemId, snippet }
      );
