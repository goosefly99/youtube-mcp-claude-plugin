...
import {
  decideHydration,
  buildNewPlaylistItemsSummary,
  type CandidateHydrationOutcome,
} from "../tools/get-new-playlist-items.js";
...
function state(overrides: Partial<VideoHydrationState>): VideoHydrationState {
  return {
    videoId: "v1",
    hasVideoRow: false,
    metadataStatus: null,
...
describe("decideHydration", () => {
  ...
  it("metadata_status null → needsHydrate=true, reason=metadata-incomplete", () => {
    const d = decideHydration(
      state({ hasVideoRow: true, metadataStatus: null })
    );
  ...
  it("metadata_status pending → needsHydrate=true, reason=metadata-incomplete", () => {
    const d = decideHydration(
      state({ hasVideoRow: true, metadataStatus: "pending" })
    );
    assert.deepStrictEqual(d, {
      needsHydrate: true,
      reason: "metadata-incomplete",
  ...
  it("metadata_status failed → needsHydrate=true, reason=metadata-incomplete", () => {
    const d = decideHydration(
      state({ hasVideoRow: true, metadataStatus: "failed" })
    );
    assert.deepStrictEqual(d, {
      needsHydrate: true,
      reason: "metadata-incomplete",
...
describe("getVideoHydrationStates", () => {
  ...
  it("integrates with decideHydration to produce the diff set", () => {
    ...
    const needs = candidates.filter(
      (id) => decideHydration(states.get(id)!).needsHydrate
    );
