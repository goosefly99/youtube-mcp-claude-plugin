...
function makeDetails(overrides: Partial<VideoDetails>): VideoDetails {
  return {
...
describe("channel_id persistence", () => {
  ...
  it("persists non-null channelId into videos.channel_id", () => {
    ...
    const row = db
      ...
      .get("chanTest1") as { channel_id: string | null } | undefined;
  ...
  it("persists null channelId as NULL without throwing", () => {
    ...
    const row = db
      .prepare("SELECT channel_id FROM videos WHERE video_id = ?")
      .get("chanTest2") as { channel_id: string | null } | undefined;
  ...
  it("COALESCE preserves an already-populated channel_id when a null upsert follows", () => {
    ...
    upsertVideo(
      db,
      makeDetails({ videoId: "chanTest3", channelId: "UCpreserved" }),
      "test"
    );
    // Second upsert with channel_id null — should NOT overwrite the existing value
    upsertVideo(
      db,
      makeDetails({ videoId: "chanTest3", channelId: null }),
      "test"
    );

    const row = db
      .prepare("SELECT channel_id FROM videos WHERE video_id = ?")
      .get("chanTest3") as { channel_id: string | null } | undefined;

    assert.ok(row, "row must exist");
    assert.strictEqual(
      row.channel_id,
      "UCpreserved",
      "channel_id must be preserved by COALESCE"
    );
  });
});
