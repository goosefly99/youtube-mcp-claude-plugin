...
export function createServer(): McpServer {
  ...
  try {
    ...
    const transcriptCount = (db.prepare("SELECT COUNT(*) as n FROM transcripts").get() as { n: number }).n;
    const playlistCount = (db.prepare("SELECT COUNT(*) as n FROM playlists").get() as { n: number }).n;
    console.error(
      `youtube-mcp: DB ready at ${getDbPath()} (videos: ${videoCount}, transcripts: ${transcriptCount}, playlists: ${playlistCount})`
    );
  } catch (err) {
    console.error(
      `youtube-mcp: DB init failed (continuing without persistence): ${err}`
    );
  }
